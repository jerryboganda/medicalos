...
| CORE-09 | Guest trial before sign-up (SHOULD) | blocked (implementation complete: bounded deterministic guest samples, answer secrecy/idempotency, hashed 24-hour token, atomic answered-progress migration, accessible login sample path; local fmt/diff checks pass; final Actions acceptance cannot start because GitHub billing/spending-limit state blocks jobs before runner steps) | `.scratch/phase-1-core09-guest-trial/`; blocked run 35445798553 |
...
| QB-04 | Confidence and assistance evidence separation | in-progress (confidence stored; QB-13 tutor hints now persist `attempts.assisted` locally and assisted evidence is excluded from `independent_count`; heavy CI acceptance pending) | run 35280682748; `.scratch/phase-1-qb13-session-tools/` |
...
| QB-12 | Qbank builder: hierarchy multi-select, four pools, counts, availability rule, presets, Quick 10 | in-progress (implementation complete locally at c41bea1; RED contracts committed first; lightweight fmt/diff checks pass; heavy acceptance remains unverified because the implementation commit is not on the remote branch from this execution path, and the prior RED Actions run was billing/spending-limit blocked before runner steps) | .scratch/phase-1-qb12-builder/; run 35280682748; blocked RED run 35433616524 |
| QB-13 | Session tools baseline: calculator, converter, text size, hint, auto-submit warnings, submission summary | blocked (implementation complete locally: shared authenticated calc-engine endpoint, responsive tool tray, client converter, exactly four persisted text sizes, tutor-only assisted hints, timed warning, assisted submission count, validation/accessibility contracts; fmt/diff checks pass; heavy acceptance run 35436593968 failed before runner steps because of GitHub Actions billing/spending-limit state) | `.scratch/phase-1-qb13-session-tools/`; runs 35366131810, 35367410576; blocked run 35436593968 |
...
| UX-01 | Touch-first session workspace: gestures, tool tray, navigator, Focus Mode | implemented; heavy browser/build acceptance pending (free previous/next, navigator states/filters/jump, submission counts, draft restore, elimination, touch swipe/long-press, Focus Mode + wake lock implemented; no local compiler claim because dependencies are not installed) | `.scratch/phase-1-ux01-session-workspace/spec.md`, `tests/e2e/session-workspace.spec.ts`; GitHub Actions runner start currently blocked by account billing/spending limit |
| UX-02 | Desktop and web keyboard map and fullscreen | implemented; heavy browser/build acceptance pending (N/Right, P/Left, F, E, H, option letters, editable-control suppression, fullscreen enter/exit, wake-lock fallback reporting implemented) | `.scratch/phase-1-ux01-session-workspace/spec.md`, `tests/e2e/session-workspace.spec.ts`; GitHub Actions runner start currently blocked by account billing/spending limit |
| ENG-01 | Daily goal, streak with freezes, question of the day; all disableable | implemented (server-authoritative daily progress/streak/freezes + per-exam QOTD + independent preferences + Today UI; local formatting/diff gates pass; acceptance CI blocked before runner steps by account billing/spending-limit state; product-analytics emission remains OPS-05 rather than being fabricated here) | `.scratch/phase-1-eng01-engagement/`; blocked run 35439962380 |
