...
| QB-13 | Session tools baseline: calculator, converter, text size, hint, auto-submit warnings, submission summary | implementation complete locally (shared authenticated calc-engine endpoint, responsive tool tray, client converter, exactly four persisted text sizes, tutor-only assisted hints, timed warning, assisted submission count, validation/accessibility contracts; fmt/diff checks pass; heavy API/build/Playwright/browser acceptance pending GitHub Actions) | `.scratch/phase-1-qb13-session-tools/`; runs 35366131810, 35367410576 |
